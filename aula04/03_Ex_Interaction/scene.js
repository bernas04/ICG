...
var keyD = false, keyA = false, keyS = false, keyW = false, key_plus= false, key_less = false;
...
function computeFrame(time) {
    ...
    if (key_plus){
        ini_size+=0.02;
        let ttexists = true;
        const sphereGeometry = new THREE.SphereGeometry(ini_size, 32, 32);
        const sphereMaterial = new THREE.MeshPhongMaterial({ color: 'rgb(180,180,255)' });
        const sphereObject = new THREE.Mesh(sphereGeometry, sphereMaterial);
        sphereObject.name="esfera";
        sceneElements.sceneGraph.add(sphereObject);
        sphereObject.translateX(-1.2).translateY(0.5).translateZ(-0.5);
